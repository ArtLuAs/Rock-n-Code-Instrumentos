...
import { Container, Row, Col, Card, Button, Badge, Offcanvas, ListGroup, Alert, Nav, Form } from 'react-bootstrap';
import { useOutletContext } from 'react-router-dom';
import { api } from '../services/api';

// Constante com as formas de pagamento disponíveis
  ...
  const finalizarPedido = async () => {
    ...
    try {
      await api.post('/vendas', {
        clienteId: userId,
        funcionarioId: 1,
        formaPagamento: formaPagamento, // Agora envia a forma selecionada no Select
        instrumentos: carrinho.map(i => i.id),
        quantidades: carrinho.map(i => i.qtd),
      });
      setCarrinho([]);
      setShowCart(false);
      setFormaPagamento('dinheiro'); // Reseta a forma de pagamento após a compra
        ...
        <Offcanvas.Body className="d-flex flex-column">
          {carrinho.length === 0 ? (
            ...
            <>
              ...
              {/* Novo Bloco: Seleção de Forma de Pagamento */}
              <Form.Group className="mb-3 mt-4 px-1">
