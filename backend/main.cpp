...
static int intVal(const json& b, const string& key, int def = 0) {
        ...
        if (b[key].is_string())
                return atoi(b[key].get<string>().c_str());
        return def;
}
...
int main() {
        ...
        svr.Get("/api/produtos", [conn](const httplib::Request&, httplib::Response& res) {
                ...
                for (int i = 0; i < rows; i++) {
                        arr.push_back({{"id", atoi(PQgetvalue(result, i, 0))},
        ...
        svr.Get("/api/clientes/(\\d+)", [conn](const httplib::Request& req, httplib::Response& res) {
                ...
                if (PQresultStatus(result) != PGRES_TUPLES_OK || PQntuples(result) == 0) {
                        PQclear(result);
                        errResp(res, "Cliente nao encontrado", 404);
                        return;
                }
                json resp = {
        ...
        svr.Get("/api/vendas/me", [conn](const httplib::Request& req, httplib::Response& res) {
                ...
                if (clienteIdStr.empty()) {
                        errResp(res, "clienteId e obrigatorio");
                        return;
                }
        ...
        svr.Patch("/api/vendas/(\\d+)/status", [conn](const httplib::Request& req, httplib::Response& res) {
                try {
                        ...
                        if (status != "pendente" && status != "confirmado" && status != "recusado") {
                                errResp(res, "Status invalido. Use: pendente, confirmado ou recusado");
                                return;
                        }
        ...
        svr.Post("/api/vendas", [conn](const httplib::Request& req, httplib::Response& res) {
                try {
                        ...
                        bool temFunc = b.contains("funcionarioId") && !b["funcionarioId"].is_null() && intVal(b, "funcionarioId") != 0;
        ...
        svr.Get("/api/relatorio/mensal", [conn](const httplib::Request& req, httplib::Response& res) {
                ...
                if (ano.empty() || mes.empty()) {
                        errResp(res, "Parametros 'ano' e 'mes' sao obrigatorios");
                        return;
                }
                ...
                for (int i = 0; i < nVendas; i++) {
                        ...
                        if (status == "pendente")
                                recPend += total;
                        totalGeral += total;
                        vendas.push_back({{"id", atoi(PQgetvalue(rVendas, i, 0))},
                ...
                for (int i = 0; i < nTop; i++) {
                        topProdutos.push_back({{"nome", PQgetvalue(rTop, i, 0)},
                ...
                for (int i = 0; i < nVend; i++) {
                        rankingVendedores.push_back({{"vendedor", PQgetvalue(rVend, i, 0)},
